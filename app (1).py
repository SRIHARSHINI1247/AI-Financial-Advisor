...
def financial_advice(category_spending, total_spent):
    ...
    if "Food" in category_spending.index:
        ...
        if category_spending["Food"] > 2000:
            st.warning(
                "High spending on food. Try reducing food delivery expenses."
            )
        else:
            st.success(
    ...
    if "Transport" in category_spending.index:
        ...
        if category_spending["Transport"] > 1500:
            st.warning(
                "Transport expenses are high."
            )
        else:
            st.success(
                "Transport spending looks reasonable."
            )
    ...
    if "Entertainment" in category_spending.index:
        ...
        if category_spending["Entertainment"] > 1000:
            ...
            st.success(
                "Entertainment spending is balanced."
            )

    if total_spent > 5000:

        st.error(
            "Overall spending is high. Consider budgeting."
        )

    elif total_spent > 2000:

        st.info(
            "Moderate spending. Track expenses carefully."
        )
